...
export default class ProductListPresenter extends Component {
    ...
    render() {
        ...
        const { shownGroup, products, isLoading } = this.props.data;
        ...
        return (
            <div style={{minHeight: '95vh', }}>
                ...
                {/* {isLoading ? <div> loading </div> : 
                    ...
                    <PoseGroup>
                        <Div key={'pose-bitch' + shownGroup}>

                             
                                    {sortedProducts.map((product, i) => {
                                        // return <Product {...product} product={product} key={'product' + i} />
                                        // return <ProductCard {...product} product={product} key={'product' + i} />
                                        return <ChildDiv key={'product' + i + shownGroup}><ProductCard {...product} product={product} /></ChildDiv>
                                    })}
